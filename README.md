...
- **Report** — "write this up", "export a PDF". Runs any missing analysis first, then
  builds the document. See below.
...
Ask for one in plain language — "write me a report on this", "export a PDF summary",
"put that together as a document" — and the agent calls `create_report`. A download
button appears under its reply.

The report is assembled from what the conversation already produced: the charts, the
result tables, and the list of tables actually queried. The model supplies only the
narrative (title, introduction, findings), never the data. By default it covers the whole
session, so a report asked for at the end includes analysis from earlier turns; pass
`scope="last"` for just the most recent question. SQL is omitted unless the user asks for
methodology.

### Why charts are re-rendered, not screenshotted

Plotly's static image export (`fig.to_image`) needs kaleido **and** a real Chrome
install. That is a heavy and often impossible dependency on a server, and it fails at
runtime rather than install time. So each chart records its spec and source rows when it
is created, and the report redraws it with matplotlib — pure Python, no browser.

If kaleido and Chrome are present the report uses them first, giving charts identical to
the ones on screen; otherwise it falls back silently. To enable that path:
...
The matplotlib fallback covers every chart type except Sankey, which has no good
matplotlib equivalent and is drawn as a ranked horizontal bar of the flows instead. If a
chart cannot be drawn at all, the report prints its underlying data as a table rather
than leaving a gap.

### Report layout

Title and timestamp, then a data-sources table listing only the tables the queries
touched, then key findings, then one section per exchange with its question, answer,
optional SQL and charts. Tables are capped at 25 rows and 8 columns with a note stating
what was truncated, headings are glued to the content beneath them so none is stranded at
a page foot, and axis ticks are formatted as `300.0M` rather than matplotlib's default
`1e8` offset.
...
## Notes on JSON safety

Every tool return passes through `json_safe()` before going back to the model. This is
not defensive boilerplate — `NaN` is not valid JSON, and the Gemini API rejects a payload
containing it with a `400 INVALID_ARGUMENT`. Two pandas behaviours make this easy to get
wrong:

- `df.where(df.notna(), None)` looks like it strips NaN, but on a float column pandas
  stores `None` back as `NaN`, so the scrub silently does nothing.
- In pandas 3.0, `astype(str)` propagates NaN as missing rather than producing the
  literal string `"nan"`, so stringifying a frame no longer launders it either.

Sanitising happens at the value level after `to_dict()`, where dtype coercion can't
undo it, and covers NaN, ±inf, `NaT`, `pd.NA`, numpy scalars, `Decimal`, `bytes` and
datetimes. Sheets with sparse blocks — a column of prose beside mostly-empty cells — hit
this constantly, so it matters most on exactly the messy workbooks this app targets.
...
- Reports are written to `/tmp/viz_reports` (override with `VIZ_REPORT_DIR`). Files are
  not cleaned up automatically, and a download link from an old message breaks if the
  directory is cleared.
- The kaleido/Chrome chart path is implemented but untested here — no Chrome was
  available in the build environment. The matplotlib fallback is the tested path.
